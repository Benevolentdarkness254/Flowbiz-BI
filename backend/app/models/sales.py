...
class Customer(db.Model):
    ...
    customer_type = db.Column(
        db.Enum(CustomerType, values_callable=lambda obj: [e.value for e in obj]),
    ...
    transactions = db.relationship(
        "SaleTransaction", back_populates="customer", lazy="dynamic"
    )
...
class Invoice(db.Model):
    ...
    transaction_id = db.Column(
        db.Integer,
        db.ForeignKey("sale_transactions.transaction_id"),
        nullable=False,
        unique=True,
    )
    ...
    kra_status = db.Column(
        ...
        default=KRAStatus.NOT_SUBMITTED,
    )
...
class OutboundDelivery(db.Model):
    ...
    # GPS coordinates — populated from customer location at delivery creation
    latitude = db.Column(db.Numeric(10, 7))
    longitude = db.Column(db.Numeric(10, 7))
    # Auto-estimated delivery time in minutes based on zone distance
    eta_minutes = db.Column(db.Integer)
    status = db.Column(
        db.Enum(DeliveryStatus, values_callable=lambda obj: [e.value for e in obj]),
