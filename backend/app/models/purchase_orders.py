...
class PurchaseOrder(db.Model):
    ...
    status = db.Column(
        db.Enum(POStatus, values_callable=lambda obj: [e.value for e in obj]),
...
class PurchaseOrderItem(db.Model):
    ...
    purchase_order_id = db.Column(
        db.Integer, db.ForeignKey("purchase_orders.purchase_order_id"), nullable=False
    )
    ...
    subtotal = db.Column(
        db.Numeric(12, 2), Computed("quantity * unit_price", persisted=True)
    )
