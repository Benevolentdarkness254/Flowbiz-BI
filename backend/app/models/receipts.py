...
class Receipt(db.Model):
    ...
    product_id          = db.Column(db.Integer, db.ForeignKey('products.product_id'))  # for deposits
    ...
    kra_status          = db.Column(db.Enum(KRAStatus), default=KRAStatus.NOT_REQUIRED)
...
class ReceiptPrintLog(db.Model):
    ...
    dispatch_channel = db.Column(db.Enum(DispatchChannel), nullable=False)
    dispatched_to    = db.Column(db.String(100))
    dispatched_by    = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    template_version = db.Column(db.String(20))
    digital_ref      = db.Column(db.String(200))
    status           = db.Column(db.Enum('queued', 'sent', 'delivered', 'failed',
                                          name='dispatch_status_enum'), default='queued')
