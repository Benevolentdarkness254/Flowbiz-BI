...
def approve_po(po_id):
    ...
    Sets status to APPROVED, records who approved and when.
    Only works for POs in PENDING_APPROVAL state.
    """
    # get_jwt_identity() returns the string user_id stored in the JWT
...
def decline_po(po_id):
    """
    Decline a pending purchase order with a required reason.
    Sets status to DECLINED and records the rejection reason.
    """
    data = request.get_json() or {}
    user_id = int(get_jwt_identity())
    po = db.session.get(PurchaseOrder, po_id)
